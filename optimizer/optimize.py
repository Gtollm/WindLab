# CMA-ES optimizer over shape_gen bodies. Needs release windlab and eval-stl "Cd=..." stdout.
...
class ProgressTracker:
    def __init__(self, max_iter: int, pop_size: int, mode: str):
        ...
        self.mode = mode
        self.total_est = max_iter * pop_size
        self.iter = 0
        self.eval = 0
    ...
    def _print_iter(self, sigma: float) -> None:
        ...
        line = (
            ...
            f"   "  # pad so \r overwrites a longer previous line
        )
...
def _pick_binary() -> Path:
    # release build only; debug eval-stl is far too slow
...
def evaluate(
    ...
    *,
    ...
    # +inf marks failed runs so CMA-ES discards the candidate
...
def main():
    ...
    parser.add_argument("--volume", type=float, default=sg.V_TARGET,
                        help=f"Target volume in m^3 (default: {sg.V_TARGET:.2e})")
